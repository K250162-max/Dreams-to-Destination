...
var consultationRequests = mysqlTable("consultationRequests", {
  id: int("id").autoincrement().primaryKey(),
...
var contactEnquiries = mysqlTable("contactEnquiries", {
  id: int("id").autoincrement().primaryKey(),
  name: varchar("name", { length: 120 }).notNull(),
  email: varchar("email", { length: 320 }).notNull(),
...
async function createContactEnquiry(input) {
...
// server/routers.ts
var emailSchema = z2.string().trim().email().max(320);
var phoneSchema = z2.string().trim().min(7).max(40).regex(/^[+()\-\s\d]+$/, "Enter a valid phone number");
var leadProtectionSchema = z2.object({
...
var appRouter = router({
  ...
  leads: router({
    createConsultation: publicProcedure.input(
      leadProtectionSchema.extend({
        name: z2.string().trim().min(2).max(120),
        email: emailSchema,
        phone: phoneSchema,
        visaType: z2.string().trim().min(2).max(80),
        preferredDate: z2.string().regex(/^\d{4}-\d{2}-\d{2}$/),
        preferredTime: z2.string().trim().min(2).max(40),
    ...
    createEnquiry: publicProcedure.input(
      leadProtectionSchema.extend({
        name: z2.string().trim().min(2).max(120),
        email: emailSchema,
        phone: phoneSchema.optional().or(z2.literal("")),
        subject: z2.string().trim().min(3).max(160),
        message: z2.string().trim().min(10).max(2500)
      })
    ).mutation(async ({ input }) => {
